...
import { FXAAShader } from "https://cdn.skypack.dev/three@0.136/examples/jsm/shaders/FXAAShader";
...
class FirstPersonCamera {
  ...
  updateCamera_(_) {
    ...
    this.camera_.position.y += Math.sin(this.headBobTimer_ * 10) * 0.08;
...
class FirstPersonCameraDemo {
  ...
  initializeScene_() {
    ...
    // For detecting the Raycasted Meshes
    var draggable = new THREE.Object3D();

    // Creating video for videoScreen
    ...
    this.scene_.background = new THREE.Color(0xB9F3FC);
    const fogColor = new THREE.Color(0xB9F3FC);
    const fogNear = 10;
    const fogFar = 50;
    this.scene_.fog = new THREE.Fog(fogColor, fogNear, fogFar);
    ...
    // Texture for the imageScreen
    const imageLoader = new THREE.TextureLoader();
    const imgmaxAnisotropy = 256;
    const image = mapLoader.load('resources/Wallpaper.jpg');
    image.anisotropy = imgmaxAnisotropy;
    ...
    videoScreen.position.set(-5,3.1,-15);
    videoScreen.rotation.y = Math.PI/7;
    videoScreen.receiveShadow = true;
    this.scene_.add(videoScreen);

    videoScreen.userData.draggable = true;
    videoScreen.userData.name = 'videoScreen';

    

    const imageScreen = new THREE.Mesh(
      new THREE.PlaneGeometry(10,5,10,10),
      new THREE.MeshBasicMaterial({map: image, overdraw: true, side:THREE.DoubleSide})
    );
    imageScreen.castShadow = true;
    imageScreen.position.set(5,3.1,-15);
    imageScreen.rotation.y = -Math.PI/7;
    imageScreen.receiveShadow = true;
    this.scene_.add(imageScreen);

    imageScreen.userData.draggable = true;
    imageScreen.userData.name = 'imageScreen';
    ...
    window.addEventListener('click', event => {
      ...
      clickMouse.y = ( event.clientY / window.innerWidth ) * 2 + 1;

      raycaster.setFromCamera( clickMouse , this.camera_ );
      const found = raycaster.intersectObjects( this.scene_.children );
      if( found.length > 0 ){
        const intersectedObject = found[0].object;

        // Now you have access to the intersected object
        if( intersectedObject.userData.name == 'videoScreen')
        {
          console.log("videoScreen")
        }
        else if( intersectedObject.userData.name == 'imageScreen')
        {
    ...
    const meshes = [
      plane, imageScreen, videoScreen, wall1, wall2, wall3, wall4];
